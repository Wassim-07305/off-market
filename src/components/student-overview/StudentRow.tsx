...
export function StudentRow({ student, onClick }: StudentRowProps) {
  const totalItems = student.formations.reduce(
    (sum, f) => sum + (f.progress?.total_items ?? 0),
    0,
  );
  const completedItems = student.formations.reduce(
    (sum, f) => sum + (f.progress?.completed_items ?? 0),
    0,
  );
      ...
      <td className="px-4 py-3 text-right">
        <span className="text-xs text-muted-foreground">
          {student.last_seen_at
            ? formatRelativeDate(student.last_seen_at)
            : "—"}
        </span>
      </td>
    </tr>
