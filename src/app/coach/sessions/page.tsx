...
const SESSION_TYPE_CONFIG: Record<SessionType, { label: string; icon: typeof Video; color: string }> = {
  individual: { label: "Individuel", icon: Video, color: "text-blue-500 bg-blue-500/10" },
  group: { label: "Groupe", icon: Users, color: "text-purple-500 bg-purple-500/10" },
  emergency: { label: "Urgence", icon: AlertTriangle, color: "text-red-500 bg-red-500/10" },
};

export default function CoachSessionsPage() {
  const { sessions, isLoading, createSession, completeSession, updateSession } = useSessions();
      ...
      <motion.div variants={fadeInUp} transition={defaultTransition}>
        ...
        {isLoading ? (
          ...
          <div className="bg-surface border border-border rounded-xl p-8 text-center">
            ...
            <p className="text-sm text-muted-foreground">Aucune seance planifiee</p>
          </div>
        ) : (
          <div className="space-y-2">
            {upcoming.map((session) => (
              <SessionCard
                ...
                onCancel={() => updateSession.mutate({ id: session.id, status: "cancelled" })}
              />
            ))}
          </div>
        )}
...
function SessionStatusBadge({ status }: { status: string }) {
  const config: Record<string, { label: string; className: string }> = {
    scheduled: { label: "Planifiee", className: "bg-blue-500/10 text-blue-600" },
    completed: { label: "Terminee", className: "bg-emerald-500/10 text-emerald-600" },
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-surface border border-border rounded-2xl w-full max-w-lg m-4">
        <div className="flex items-center justify-between p-6 border-b border-border">
          <h2 className="text-lg font-semibold text-foreground">Nouvelle seance</h2>
        ...
        <div className="p-6 space-y-4">
          ...
          <div>
            ...
            <div className="flex gap-2">
              {(Object.entries(SESSION_TYPE_CONFIG) as [SessionType, typeof SESSION_TYPE_CONFIG.individual][]).map(
                ([type, config]) => (
                  <button
                    key={type}
