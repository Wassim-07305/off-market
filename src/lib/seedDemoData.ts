...
export async function seedDemoData(adminUserId: string): Promise<string> {
  ...
  try {
    ...
    if (rpcError)
      throw new Error(`RPC create_demo_profiles: ${rpcError.message}`);
    ...
    await supabase
      .from("profiles")
      .update({ last_seen_at: isoTimestampHoursAgo(2) })
      .eq("id", karimId);
    await supabase
      .from("profiles")
      .update({ last_seen_at: isoTimestampHoursAgo(48) })
      .eq("id", emmaId);
    await supabase
      .from("profiles")
      .update({ last_seen_at: isoTimestampHoursAgo(120) })
      .eq("id", thomasId);
    await supabase
      .from("profiles")
      .update({ last_seen_at: isoTimestampHoursAgo(1) })
      .eq("id", sarahId);
    await supabase
      .from("profiles")
      .update({ last_seen_at: isoTimestampHoursAgo(6) })
      .eq("id", lucasId);
    ...
    const clientsToInsert = [
      {
        ...
        status: "actif",
        notes: "Programme fitness haut de gamme",
        created_by: adminUserId,
      },
      {
        name: "Digital Academy",
        email: "hello@digitalacademy.io",
        phone: "06 23 45 67 89",
        status: "actif",
        notes: "Formation marketing digital",
        created_by: adminUserId,
      },
      {
        name: "Invest Mastery",
        email: "team@investmastery.fr",
        phone: "06 34 56 78 90",
        status: "actif",
        notes: "Coaching investissement immobilier",
        created_by: adminUserId,
      },
      {
        name: "MindSet Lab",
        email: "info@mindsetlab.com",
        phone: "06 45 67 89 01",
        status: "actif",
        notes: "Développement personnel et mindset",
        created_by: adminUserId,
      },
      {
        name: "E-Com Empire",
        email: "support@ecomempire.fr",
        phone: "06 56 78 90 12",
        status: "actif",
        notes: "Formation e-commerce Shopify",
        created_by: adminUserId,
      },
      {
        name: "Scale Agency",
        email: "bonjour@scaleagency.fr",
      ...
      {
        name: "Crypto Mentor",
        email: "contact@cryptomentor.io",
        phone: "06 78 90 12 34",
        status: "inactif",
        notes: "Formation crypto-monnaies — en pause",
        created_by: adminUserId,
      },
      {
        name: "Wellness Studio",
        email: "hello@wellnessstudio.fr",
        phone: "06 89 01 23 45",
        status: "archivé",
        notes: "Ancien client — bien-être",
        created_by: adminUserId,
      },
    ];
    ...
    for (let weekOffset = 7; weekOffset >= 0; weekOffset--) {
      ...
      for (let j = 0; j < count; j++) {
        ...
        leadsToInsert.push({
          ...
          created_at: new Date(
            new Date().getTime() - daysBack * 86400000,
          ).toISOString(),
        });
        leadIdx++;
      }
    ...
    const { data: setterData, error: setterError } = await supabase
      ...
      .select("id");
    if (setterError)
      throw new Error(`Setter activities: ${setterError.message}`);
    ...
    const financialEntries = [
      {
        type: "ca",
        label: "Paiement FitCoach Pro — Janvier",
        amount: 5000,
        client_id: fitcoachId,
        is_paid: true,
        date: monthsAgo(1),
      },
      {
        type: "ca",
        label: "Paiement Digital Academy — Janvier",
        amount: 3500,
        client_id: digitalId,
        is_paid: true,
        date: monthsAgo(1),
      ...
      {
        type: "ca",
        label: "Paiement Invest Mastery — Février",
        amount: 4500,
        client_id: investId,
        is_paid: true,
        date: daysAgo(10),
      ...
      {
        type: "ca",
        label: "Paiement E-Com Empire — Février",
        amount: 3000,
        client_id: ecomId,
        is_paid: false,
        date: daysAgo(3),
      ...
      {
        type: "prestataire",
        label: "Montage vidéo — Thomas",
        amount: 600,
        client_id: scaleId,
        is_paid: true,
        date: monthsAgo(1),
        prestataire: "Thomas Moreau",
      },
      {
        type: "prestataire",
        label: "Graphisme — freelance",
        amount: 350,
        client_id: fitcoachId,
        is_paid: true,
        date: monthsAgo(2),
        prestataire: "Marie Design",
      },
      {
        type: "prestataire",
        label: "Copywriting — freelance",
        amount: 450,
        client_id: digitalId,
        is_paid: false,
        date: daysAgo(7),
        prestataire: "Paul Writer",
      },
      {
        type: "prestataire",
        label: "Dev landing page",
        amount: 800,
        client_id: ecomId,
        is_paid: true,
        date: monthsAgo(3),
        prestataire: "DevStudio",
      },
    ];
    ...
    const paymentSchedules = [
      {
        financial_entry_id: finData[0].id,
        client_id: fitcoachId,
        amount: 1667,
      ...
      {
        financial_entry_id: finData[2].id,
        client_id: investId,
        amount: 2250,
        due_date: daysAgo(10),
        is_paid: true,
      ...
      {
        financial_entry_id: finData[3].id,
        client_id: ecomId,
        amount: 1000,
        due_date: daysAgo(3),
        is_paid: false,
    ...
    const notificationsToInsert = [
      {
        ...
        title: "Bienvenue sur Off-Market",
      ...
      },
      {
        ...
        title: "Lead Antoine Girard → call planifié",
        message: "Le statut du lead a changé.",
        is_read: true,
      ...
      },
      {
        ...
        title: "Nouveau call planifié",
        message: "Un call a été ajouté au calendrier.",
        is_read: true,
        created_at: isoTimestamp(12),
      },
      {
        user_id: adminUserId,
        type: "call_closed" as const,
        title: "Call closé — 5 000 €",
        message: "Un call a été closé.",
        is_read: true,
      ...
      },
      {
        user_id: adminUserId,
        type: "lead_status" as const,
        title: "Lead Camille Roux → en discussion",
        message: "Le statut du lead a changé.",
        is_read: true,
      ...
      },
      {
        ...
        message: "Le rapport de janvier est prêt.",
        is_read: true,
      ...
      },
      {
        user_id: adminUserId,
        type: "general" as const,
        title: "Nouvelle action requise",
        message: "Vérifiez les relances.",
        is_read: false,
      ...
      },
      {
        user_id: adminUserId,
        type: "lead_status" as const,
        title: "Lead Léa Bernard → call planifié",
        message: "Nouveau call planifié.",
        is_read: false,
      ...
      },
      {
        user_id: adminUserId,
        type: "new_call" as const,
        title: "Nouveau call demain",
        message: "Call avec E-Com Empire.",
        is_read: false,
        created_at: new Date().toISOString(),
      },
      {
        ...
        message: "Vérifiez les échéanciers.",
        is_read: false,
        created_at: new Date().toISOString(),
      },
      {
        user_id: adminUserId,
        type: "general" as const,
        title: "Objectif mensuel à 80%",
        message: "Vous êtes proche de l'objectif !",
        is_read: false,
        created_at: new Date().toISOString(),
      },
    ];
    ...
    await supabase.from("channel_members").insert(
      [adminUserId, sarahId, lucasId, karimId, emmaId].map((uid) => ({
        channel_id: vipChannel.id,
        user_id: uid,
      })),
    );
      ...
      const gMsgs = [
        {
          ...
          content:
            "Bienvenue sur Off-Market ! N'hésitez pas à poser vos questions ici.",
          created_at: isoTimestamp(10),
        },
        {
          channel_id: generalChannel.id,
          sender_id: karimId,
          content: "Merci ! Ravi d'être ici.",
          created_at: isoTimestamp(9),
        },
        {
          channel_id: generalChannel.id,
          sender_id: emmaId,
          content: "Super plateforme, j'ai hâte de commencer les formations !",
          created_at: isoTimestamp(8),
        },
        {
          channel_id: generalChannel.id,
          sender_id: sarahId,
          content: "N'hésitez pas si vous avez des questions.",
          created_at: isoTimestamp(7),
        },
        {
          channel_id: generalChannel.id,
          sender_id: lucasId,
          content: "Je suis là aussi pour vous aider.",
          created_at: isoTimestamp(6),
        },
        {
          channel_id: generalChannel.id,
          sender_id: adminUserId,
          content:
            "Rappel : les formations sont disponibles dans l'onglet Formations.",
          created_at: isoTimestamp(3),
        },
        {
          channel_id: generalChannel.id,
          sender_id: karimId,
          content: "J'ai commencé le module 1, c'est top !",
          created_at: isoTimestamp(2),
        },
        {
          channel_id: generalChannel.id,
          sender_id: thomasId,
          content: "Pareil, le contenu est vraiment qualitatif.",
          created_at: isoTimestamp(1),
        },
      ];
    ...
    const vMsgs = [
      {
        channel_id: vipChannel.id,
        sender_id: adminUserId,
        content: "Bienvenue dans le groupe VIP !",
        created_at: isoTimestamp(5),
      },
      {
        channel_id: vipChannel.id,
        sender_id: sarahId,
        content: "N'oubliez pas le call de groupe demain à 14h.",
        created_at: isoTimestamp(4),
      },
      {
        channel_id: vipChannel.id,
        sender_id: karimId,
        content: "Noté, merci Sarah !",
        created_at: isoTimestamp(3),
      },
      {
        channel_id: vipChannel.id,
        sender_id: emmaId,
        content: "J'ai une question sur le closing, on en parle au call ?",
        created_at: isoTimestamp(2),
      },
      {
        channel_id: vipChannel.id,
        sender_id: adminUserId,
        content: "Bien sûr Emma, on fera un point.",
        created_at: isoTimestamp(1),
      },
    ];
    ...
    const directMsgs = [
      [
        { s: adminUserId, c: "Salut Karim, comment avance ta formation ?" },
        { s: karimId, c: "Ça avance bien, 80% du module 1 !" },
        { s: adminUserId, c: "Super, continue !" },
      ],
      [
        {
          s: adminUserId,
          c: "Emma, bienvenue ! Tu as pu accéder aux formations ?",
        },
        { s: emmaId, c: "Oui tout est bon !" },
        { s: adminUserId, c: "Parfait." },
      ],
      [
        { s: adminUserId, c: "Thomas, tu en es où ?" },
        { s: thomasId, c: "J'ai pris du retard, je m'y remets." },
        { s: adminUserId, c: "Pas de souci, avance à ton rythme." },
      ],
    ];
    for (let i = 0; i < directMsgs.length; i++) {
      const chId = ids.channelIds[i + 1];
      const msgs = directMsgs[i].map((m, j) => ({
        channel_id: chId,
        sender_id: m.s,
        content: m.c,
        created_at: isoTimestamp(5 - j),
      }));
    ...
    const { data: f1, error: f1Err } = await supabase
      ...
      .insert({
        ...
        description:
          "Apprenez les bases : acquisition, closing, fidélisation et scaling.",
        status: "published",
        sort_order: 0,
        created_by: adminUserId,
      })
    ...
    // F1 modules
    const { data: f1Mods, error: f1ModErr } = await supabase
      .from("formation_modules")
      .insert([
        {
          course_id: f1.id,
          title: "Introduction au coaching",
          description: "Les bases et le mindset.",
          sort_order: 0,
        },
        {
          course_id: f1.id,
          title: "Acquisition de clients",
          description: "Stratégies organique et payante.",
          sort_order: 1,
        },
        {
          course_id: f1.id,
          title: "Techniques de closing",
          description: "Closer efficacement.",
          sort_order: 2,
        },
        {
          course_id: f1.id,
          title: "Fidélisation et scaling",
          description: "Garder ses clients et grandir.",
          sort_order: 3,
        },
      ])
      .select("id");
    ...
    // F2 modules
    const { data: f2Mods, error: f2ModErr } = await supabase
      .from("formation_modules")
      .insert([
        {
          course_id: f2.id,
          title: "Psychologie du closing",
          description: "Mécanismes de décision.",
          sort_order: 0,
        },
        {
          course_id: f2.id,
          title: "Scripts et frameworks",
          description: "Les scripts qui convertissent.",
          sort_order: 1,
        },
      ])
      .select("id");
    ...
    const { data: f1Items, error: f1ItErr } = await supabase
      .from("module_items")
      .insert([
        {
          module_id: f1Mods[0].id,
          title: "Qu'est-ce que le coaching business ?",
          content_type: "video",
        ...
        },
        {
          ...
          video_url: "https://example.com/guide.pdf",
          sort_order: 3,
        },
        {
          module_id: f1Mods[1].id,
        ...
        {
          module_id: f1Mods[1].id,
          title: "LinkedIn pour les coachs",
          content_type: "video",
          video_url: demoVideoUrl,
        ...
        {
          module_id: f1Mods[1].id,
          title: "Templates messages DM",
          content_type: "document",
          video_url: "https://example.com/templates.pdf",
          sort_order: 3,
        },
        {
          module_id: f1Mods[2].id,
          title: "Le call de découverte",
          content_type: "video",
          video_url: demoVideoUrl,
        ...
        {
          module_id: f1Mods[3].id,
          title: "Scaling : de 1 à 10 clients",
          content_type: "video",
          video_url: demoVideoUrl,
        ...
        {
          module_id: f1Mods[3].id,
          title: "Checklist scaling (PDF)",
          content_type: "document",
          video_url: "https://example.com/checklist.pdf",
          sort_order: 3,
        },
      ])
      .select("id");
    ...
    // F2 items (6)
    const { data: f2Items, error: f2ItErr } = await supabase
      .from("module_items")
      .insert([
        {
          module_id: f2Mods[0].id,
        ...
        {
          module_id: f2Mods[1].id,
          title: "Script call de closing",
          content_type: "document",
          video_url: "https://example.com/script.pdf",
          sort_order: 0,
        },
        {
          module_id: f2Mods[1].id,
        ...
        },
        {
          ...
          content_type: "document",
          video_url: "https://example.com/exercices.pdf",
          sort_order: 2,
        },
      ])
      .select("id");
    ...
    const completions = [
      ...f1Items
        .slice(0, 12)
        .map((it) => ({ item_id: it.id, user_id: karimId })), // 80%
      ...f1Items.slice(0, 6).map((it) => ({ item_id: it.id, user_id: emmaId })), // 40%
      ...f1Items
        .slice(0, 2)
        .map((it) => ({ item_id: it.id, user_id: thomasId })), // ~13%
    ];
    ...
    const closerCallsToInsert = Array.from({ length: 20 }, (_, i) => {
      ...
      return {
        ...
        debrief: isClosed
          ? `Call closé avec succès — ${2000 + i * 500} €`
          : "Prospect pas convaincu, relancer dans 1 mois.",
    ...
    const socialTitles = [
      "Témoignage client FitCoach",
      "Behind the scenes coaching",
      "Les 5 erreurs du débutant",
      "Résultats du mois",
      "FAQ coaching business",
      "Routine matinale du coach",
      "Comment closer en 5 min",
      "Story day-in-the-life",
      "Avant/Après transformation",
      "Tips LinkedIn",
      "Reel motivation",
      "Carrousel objections",
      "Post résultats Q1",
      "Story Q&A",
      "Reel process closing",
      "Contenu éducatif",
      "Post valeur ajoutée",
      "Reel lifestyle",
      "Carrousel méthode",
    ...
    const socialToInsert = socialTitles.map((title, i) => ({
      ...
      video_type:
        socialFormats[i % socialFormats.length] === "reel"
    ...
    const { data: igStatData, error: igStatError } = await supabase
      .from("instagram_post_stats")
      .insert(igPostStats)
      .select("id");
    if (igStatError)
      throw new Error(`Instagram post stats: ${igStatError.message}`);
    ...
    return [
      ...
      "15 notifications",
      "4 canaux + ~30 messages",
      "2 formations (15+6 items)",
      "20 completions",
      "20 appels closer",
      "20 contenus sociaux",
      "2 comptes Instagram + 15 stats posts",
    ].join(", ");
  } catch (error) {
