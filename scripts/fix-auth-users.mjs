...
const ID_MAPPING = {
  "a1111111-1111-1111-1111-111111111111": "d7299124-625b-4337-849d-b73474fcc8f3", // sophie
  "a2222222-2222-2222-2222-222222222222": "e99e9e44-391b-41f8-bfbd-633d3f218678", // lucas
  "a3333333-3333-3333-3333-333333333333": "dbbf2093-f17f-4910-88a2-4df92626baf4", // marie
  "a4444444-4444-4444-4444-444444444444": "1f293b75-46bc-490c-8407-b2b0d81eb35c", // thomas
  "a5555555-5555-5555-5555-555555555555": "57bf2491-43b7-4afd-bfb4-dc2d695dac91", // emma
  "a6666666-6666-6666-6666-666666666666": "14c3f658-1e1c-4ae1-8305-1a124c9ce8f7", // hugo
  "a7777777-7777-7777-7777-777777777777": "85ade98a-7e4c-4646-8de3-3050fb2ca273", // chloe
  "a8888888-8888-8888-8888-888888888888": "1c2e23ec-3d92-4035-bf9b-967f0720978c", // nathan
  "a9999999-9999-9999-9999-999999999999": "7b07715a-a313-41cd-9535-9e191bdba25f", // lea
  "aa111111-1111-1111-1111-111111111111": "f887512f-c1b0-44a1-a1ff-d269f93e983c", // julien
  "aa222222-2222-2222-2222-222222222222": "c99ab383-e0e8-4820-a06e-2c9c82898564", // camille
  "aa333333-3333-3333-3333-333333333333": "661587a5-6e41-4a11-99ed-ec47d50268ab", // alex
  "aa444444-4444-4444-4444-444444444444": "1d805e18-1296-4a3c-bd76-3f89f6c3ca73", // sarah
  "aa555555-5555-5555-5555-555555555555": "dea9c673-bb9a-4bc0-8891-e15d9b9174a0", // maxime
  "aa666666-6666-6666-6666-666666666666": "90a6598d-b5d0-4fc3-b9fe-cf4e55999f8c", // ines
};
...
async function run() {
  ...
  try {
    ...
    for (const newId of newIds) {
      ...
      await client.query("DELETE FROM student_details WHERE profile_id = $1", [newId]);
    ...
    for (const [oldId, newId] of Object.entries(ID_MAPPING)) {
      // Update profile ID
      const pResult = await client.query("UPDATE profiles SET id = $1 WHERE id = $2", [newId, oldId]);
      if (pResult.rowCount > 0) {
        console.log(`  profiles: ${oldId.substring(0,8)}... -> ${newId.substring(0,8)}...`);
      }
    ...
    for (const oldId of Object.keys(ID_MAPPING)) {
      ...
      await client.query("DELETE FROM auth.refresh_tokens WHERE instance_id::text = $1", [oldId]);
    ...
    for (const [newId, email] of Object.entries(EMAIL_MAPPING)) {
      await client.query("UPDATE auth.users SET email = $1 WHERE id = $2", [email, newId]);
    ...
    for (const email of ["sophie@offmarket.fr", "marie@test.fr", "lucas@offmarket.fr", "thomas@test.fr"]) {
      const { error } = await testClient.auth.signInWithPassword({ email, password: "demo123456" });
      console.log(`  ${email}: ${error ? `FAIL: ${error.message}` : "SUCCESS"}`);
    }

    // Clean up testuser
    const { rows } = await client.query("SELECT id FROM auth.users WHERE email = 'testuser@offmarket.fr'");
    if (rows.length > 0) {
      await client.query("DELETE FROM student_details WHERE profile_id = $1", [rows[0].id]);
